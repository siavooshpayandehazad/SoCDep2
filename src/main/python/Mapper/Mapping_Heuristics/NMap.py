...
def NMap(task_graph, arch_graph, NoCRG, CriticalRG, NonCriticalRG, SHM, logging):
    """
    ...
    if len(task_graph.nodes()) > len(arch_graph.nodes()):
        raise ValueError("Number of tasks should be smaller or equal to number of PEs")

    mapped_tasks = []
    unmapped_tasks = copy.deepcopy(task_graph.nodes())
    allocated_nodes =[]
    unallocated_nodes = copy.deepcopy(arch_graph.nodes())
    ...
    tasks_com_dict = TG_Functions.tasks_communication_weight(task_graph)
    ...
    print ("------------------")
    print ("STEP 2:")
    node_neighbors_dict = AG_Functions.node_neighbors(arch_graph, SHM)
    ...
    if Mapping_Functions.MapTaskToNode(task_graph, arch_graph, SHM, NoCRG, CriticalRG,
    ...
    while len(unmapped_tasks) > 0:
        ...
        for Task in unmapped_tasks:
            ...
            for mapped_task in mapped_tasks:
                if (Task, mapped_task) in task_graph.edges():
                    task_weight += task_graph.edge[Task][mapped_task]["ComWeight"]
                if (mapped_task, Task) in task_graph.edges():
                    task_weight += task_graph.edge[mapped_task][Task]["ComWeight"]
        ...
        if len(tasks_with_max_com_to_mapped) > 1:
            ...
            for CandidateTask in tasks_with_max_com_to_mapped:
                ...
                for unmapped_task in unmapped_tasks:
                    if (Task, unmapped_task) in task_graph.edges():
                        task_weight += task_graph.edge[Task][unmapped_task]["ComWeight"]
                    if (unmapped_task, Task) in task_graph.edges():
                        task_weight += task_graph.edge[unmapped_task][Task]["ComWeight"]
        ...
        for unallocated_node in unallocated_nodes:
            ...
            for mapped_task in mapped_tasks:
                com_weight = 0
                if (chosen_task, mapped_task) in task_graph.edges():
                    # print ("TASK CONNECTED TO MAPPED TASK:", mapped_task)
                    com_weight += task_graph.edge[chosen_task][mapped_task]["ComWeight"]
                    destination_node = task_graph.node[mapped_task]['Node']
                    ...
                    if Calculate_Reachability.IsDestReachableFromSource(NoCRG, unallocated_node, destination_node):
                        ...
                        reachable = False
                elif (mapped_task, chosen_task) in task_graph.edges():
                    # print ("TASK CONNECTED TO MAPPED TASK:", mapped_task)
                    com_weight += task_graph.edge[mapped_task][chosen_task]["ComWeight"]
                    destination_node = task_graph.node[mapped_task]['Node']
                    # here we check if this node is even reachable from the chosen node?
    ...
    for node_id_1 in range(0, len(arch_graph.nodes())-1):
        for node_id_2 in range(node_id_1+1, len(arch_graph.nodes())-1):
            pass
            # Save current mapping in an array
            # Also save the mapping's csomm_cost in a variable
            comm_cost = calculate_com_cost(task_graph)

            # Swap (node_id_1 , node_id_2)
            swap_nodes(task_graph, arch_graph, SHM, NoCRG, CriticalRG, NonCriticalRG, node_id_1, node_id_2, logging)
            ...
            comm_cost_new = calculate_com_cost(task_graph)
            ...
            if comm_cost_new < comm_cost:
                ...
                swap_nodes(task_graph, arch_graph, SHM, NoCRG, CriticalRG, NonCriticalRG,
                           node_id_2, node_id_1, logging)
            # Reset the comm_cost after each swapping

    # End of Swapping phase
    print "SWAP PROCESS FINISHED..."
    Scheduler.ScheduleAll(task_graph, arch_graph, SHM, True, False, logging)
    return task_graph, arch_graph


def calculate_com_cost(task_graph):
    """
    ...
    for edge in task_graph.edges():
        task_1 = edge[0]
        task_2 = edge[1]
        node_1 = task_graph.node[task_1]['Node']
        node_2 = task_graph.node[task_2]['Node']
        com_weight = task_graph.edge[edge[0]][edge[1]]["ComWeight"]
        manhatan_distance = AG_Functions.manhattan_distance(node_1, node_2)
        cost += manhatan_distance * com_weight
    return cost


def swap_nodes(task_graph, arch_graph, SHM, NoCRG, critical_routing_graph,
    ...
    """
    if len(arch_graph.node[node_1]['PE'].MappedTasks) == 0 or len(arch_graph.node[node_2]['PE'].MappedTasks) == 0:
        raise ValueError("at least one of selected nodes for swapping doesnt have any tasks on it. ")

    task_1 = arch_graph.node[node_1]['PE'].MappedTasks[0]
    task_2 = arch_graph.node[node_2]['PE'].MappedTasks[0]
    Mapping_Functions.RemoveTaskFromNode(task_graph, arch_graph, NoCRG, critical_routing_graph,
                                         non_critical_routing_graph, task_1, node_1, logging)
    Mapping_Functions.RemoveTaskFromNode(task_graph, arch_graph, NoCRG, critical_routing_graph,
                                         non_critical_routing_graph, task_2, node_2, logging)
    if not Mapping_Functions.MapTaskToNode(task_graph, arch_graph, SHM, NoCRG, critical_routing_graph,
