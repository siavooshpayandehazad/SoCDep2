...
from Scheduler.Scheduling_Functions_Routers import find_last_allocated_time_on_router
from Scheduler.Scheduling_Functions_Nodes import find_last_allocated_time_on_node
from Scheduler.Scheduling_Functions_Links import find_last_allocated_time_on_link
...
def report_mapped_tasks(ag, logging):
    ...
    for link in ag.edges():
        logging.info("LINK" + str(link)+"CONTAINS THE FOLLOWING TG's Edges:" +
                     str(ag.edges[link]['MappedTasks']) + "\tWITH SCHEDULING:" +
                     str(ag.edges[link]['Scheduling']))
    return None
...
def report_scheduling_memory_usage(ag):
    ...
    for link in ag.edges():
        counter += len(ag.edges[link]['Scheduling'])
        ...
        if len(ag.node[node]['Router'].mapped_tasks) > 0:
            ...
            for task in ag.node[node]['Router'].mapped_tasks:
                ...
                if task in ag.node[node]['Router'].scheduling:
                    if tg.edges[task]['Criticality'] == 'H':
                        start_time = 0
                        end_time = 0
...
def add_links_to_drawing(tg, ag, num_of_plots, fig, node_counter, link_counter, router_counter, max_time, count):
    ...
    for Link in ag.edges():
        if len(ag.edges[Link]['MappedTasks']) > 0:
            ax1 = fig.add_subplot(num_of_plots, 1, count)
            schedule_list = []
            zorder = len(ag.edges[Link]['MappedTasks'])
            for task in ag.edges[Link]['MappedTasks']:
                ...
                if task in ag.edges[Link]['Scheduling']:
                    if tg.edges[task]['Criticality'] == 'H':
                        start_time = 0
                        end_time = 0
                        for batch_and_schedule in ag.edges[Link]['Scheduling'][task]:
                            start_time = batch_and_schedule[0]
                            task_length = batch_and_schedule[1] - start_time
